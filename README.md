...
| 03 | [公共交通空白地域の抽出](cases/03-transit-desert/) | 道路ネットワーク | 道路データ（N13）, バス停留所（P11）, 駅別乗降客数（S12）, 国勢調査250mメッシュ人口（参考） |
| 04 | [津波避難シミュレーション](cases/04-tsunami-evacuation/) | 道路ネットワーク | 道路データ（N13）, 国勢調査100mメッシュ夜間人口（参考）, 静岡県第4次地震被害想定（参考）, 指定緊急避難場所・避難施設（参考） |
